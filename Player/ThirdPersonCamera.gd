...
@export var look_ahead_scale := 0.08
@export var look_ahead_max := 0.7
...
@export var fov_sprint_add := 4.0
@export var fov_speed_threshold := 10.0
@export var fov_smooth := 8.0
...
func _physics_process(delta: float) -> void:
	...
	# Look-ahead: desloca o alvo no sentido da velocidade (mais responsivo ao correr)
	if look_ahead_enabled and player is CharacterBody3D:
		...
		var len := horz.length()
		if len > 0.5:
			var ahead = horz.normalized() * min(len * look_ahead_scale, look_ahead_max)
			target_pos += ahead

	if enable_collision_check:
		target_pos = _resolve_collision(pivot, target_pos)

	# Spring: aceleração suave em direção ao alvo (sem overshoot com damping alto)
